...
import { Plus, Search, Eye, Download, Trash2, Loader2 } from "lucide-react"
      ...
      <div className="flex-1 space-y-6">
        ...
        <div className="rounded-xl border border-gray-200 bg-white">
          {loading ? (
            ...
            <div className="py-20 text-center text-sm text-gray-500">No quotations found.</div>
          ) : (
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-100">
                  {quotations.map((q) => (
                    ...
                    >
                      <td className="px-6 py-4 text-sm font-medium text-gray-900">{q.number}</td>
                      <td className="px-6 py-4 text-sm text-gray-600">{q.client?.name}</td>
                      <td className="px-6 py-4 text-sm text-gray-600">{formatDate(q.issueDate)}</td>
                      <td className="px-6 py-4 text-sm text-gray-600">{formatDate(q.validUntil)}</td>
                      <td className="px-6 py-4 text-sm font-medium text-gray-900">{formatCurrency(q.total)}</td>
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${getStatusColor(q.status)}`}
                        >
                          {q.status}
                        </span>
                      </td>
